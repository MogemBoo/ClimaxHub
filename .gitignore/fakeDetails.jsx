...
const Details = () => {
    ...
    useEffect(() => {
        ...
        if (isSeries) {
            fetch(`http://localhost:5000/api/series/${id}/episodes`)
                .then(res => res.json())
                .then(setEpisodes)
                .catch(() => setEpisodes([]));
        } else {
    ...
    // Group episodes by season for better display
    const episodesBySeason = {};
    if (type === 'series' && Array.isArray(episodes)) {
        episodes.forEach(ep => {
                        ...
                        <div key={seasonNum} className="season-block">
                            ...
                            <div className="episodes-grid">
                                {episodesBySeason[seasonNum].map(ep => (
                                    <div className="episode-card" key={ep.episode_id}>
                                        <div className="episode-thumb">
                                            <img
                                                src={ep.thumbnail_url || '/fallback.jpg'}
                                                alt={ep.title}
